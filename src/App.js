...
function TitleScreen({onNew,onLoad}){
  return(
    <Phone>
      ...
      {/* 2. 스크롤 시 드러나는 메인 이미지 영역 */}
      <div style={{display:"flex",flexDirection:"column", background:C.bg0}}>
        {/* 💡 파일명 오류로 이미지가 안 뜰 때를 대비해 세련된 CSS 그라데이션 배경을 기본으로 깔아둡니다. */}
        ...
        {char?(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"row"}}>
            <div style={{width:"55%",height:"100%",position:"relative",overflow:"hidden",background:C.bg0,borderRight:`1px solid ${C.bdr0}`}}>
              {/* 💡 objectFit: "contain", objectPosition: "center" 로 잘림 방지 */}
...
// ── 인벤토리 화면 (9개 멀티 장착창 및 패널 상단 배치 / 1:1 비율 / 아이콘 변경) ──
    ...
    <Phone>
      ...
      <div style={{background:C.bg0,borderBottom:`1px solid ${C.bdr1}`,padding:"6px 8px",display:"flex",alignItems:"stretch",gap:4}}>
        <button onClick={onChest} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,background:C.bg2,border:`1px solid ${C.bdr2}`,borderRadius:5,padding:"5px 2px",cursor:"pointer",color:C.t1}}>
          <span style={{fontSize:14, color:C.goldL}}>✦</span><span style={{fontSize:8,color:C.t3}}>보물상자</span><span style={{fontSize:10,fontWeight:800,color:dailyLeft>0?C.goldL:C.fail}}>{dailyLeft}회</span>
        </button>
        <button onClick={onShop} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,background:C.bg2,border:`1px solid ${C.bdr2}`,borderRadius:5,padding:"5px 2px",cursor:"pointer",color:C.t1}}>
          <span style={{fontSize:14, color:C.t1}}>⚖</span><span style={{fontSize:8,color:C.t3}}>상점</span><span style={{fontSize:10,fontWeight:800}}>구매</span>
        </button>
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,background:`${C.goldD}22`,border:`1px solid ${C.goldD}55`,borderRadius:5,padding:"5px 2px"}}>
          <span style={{fontSize:14, color:C.gold}}>⬟</span><span style={{fontSize:8,color:C.t3}}>골드</span><span style={{fontSize:10,fontWeight:800,color:C.goldL}}>{gold.toLocaleString()}</span>
        </div>
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,background:C.bg2,border:`1px solid ${C.bdr2}`,borderRadius:5,padding:"5px 2px",color:C.t1}}>
          <span style={{fontSize:14, color:C.e}}>♆</span><span style={{fontSize:8,color:C.t3}}>주문서</span>
        ...
        <button onClick={onRank} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,background:C.bg2,border:`1px solid ${C.bdr2}`,borderRadius:5,padding:"5px 2px",cursor:"pointer",color:C.t1}}>
          <span style={{fontSize:14, color:C.goldL}}>♚</span><span style={{fontSize:8,color:C.t3}}>랭킹</span><span style={{fontSize:10,fontWeight:800,color:C.goldL}}>#247</span>
        </button>
      </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",borderBottom:`1px solid ${C.bdr1}`}}>
          <div style={{borderRight:`1px solid ${C.bdr0}`,background:`linear-gradient(180deg,${C.bg0},${C.bg1})`}}>
            <div style={{height:150,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",background:C.bg0}}>
              {/* 캐릭터 짤림 방지용 objectFit contain */}
              <img src={char.img} alt="" style={{height:"100%",width:"100%",objectFit:"contain",objectPosition:"center top"}}/>
  ...
  const doOpen=()=>{
    ...
    setTimeout(()=>{
      ...
      if(isGold){const tier=wRand(GOLD_TIERS); const amt=Math.floor(Math.random()*(tier.max-tier.min+1))+tier.min; setResult({type:"gold",amount:amt}); onAddGold(amt);}
        ...
        const item=rollItem();
        if(item){setResult({type:"item",item}); onAddItem(item);}
      }
...
function EnhModal({item:initItem,onClose,gold,onSpendGold,scrolls,onUseScroll,onUpdateItem}){
  ...
  const [isEnhancing, setIsEnhancing] = useState(false); // 애니메이션 상태
  ...
  const doEnh=()=>{
    ...
    onUseScroll(sk);
    setIsEnhancing(true); // 강화 이펙트 시작

    setTimeout(() => {
      setIsEnhancing(false); // 이펙트 종료 후 결과 연산
      if(Math.random()*100<prob){
        const next={...item,enhance:lv+1}; setItem(next); onUpdateItem(next); 
      } else { 
        if(lv>=9 && Math.random()*100>FK[lv]){
          const next={...item,enhance:Math.max(0,lv-1)}; setItem(next); onUpdateItem(next); 
        }
      }
    }, 1400); // 1.4초 대기 타격감
  };
    ...
    <div style={{position:"fixed",inset:0,zIndex:999,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{width:390,background:C.bg1,borderRadius:"16px 16px 0 0",padding:16,border:`1px solid ${C.bdr1}`}}>
        ...
        <div style={{background:C.bg2,padding:14,borderRadius:6,textAlign:"center",marginBottom:12,
          ...
          border: isEnhancing ? `2px solid ${C.goldL}` : `1px solid ${C.bdr1}`}}>
        ...
        <button onClick={doEnh} disabled={isEnhancing} style={{width:"100%",padding:12,background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:"#000",border:"none",borderRadius:4,fontWeight:700,cursor:"pointer"}}>
          {isEnhancing ? "강화 진행 중..." : "강화 시도"}
        </button>
      </div>
    </div>
  );
...
function ShopModal({onClose,gold,onSpendGold,onAddScrolls}){
  ...
  const ITEMS={
    weapon:[{nm:"무기 강화 주문서",img:CONSUMABLES[0].img,sub:"일반~레어 무기 강화",price:500,type:"normal"},{nm:"고급 무기 강화 주문서",img:CONSUMABLES[0].img,sub:"에픽~신화 무기 강화",price:2000,type:"adv"}],
    armor:[{nm:"방어구 강화 주문서",img:CONSUMABLES[1].img,sub:"일반~레어 방어구 강화",price:500,type:"normal"},{nm:"고급 방어구 강화 주문서",img:CONSUMABLES[1].img,sub:"에픽~신화 방어구 강화",price:2000,type:"adv"}],
    accessory:[{nm:"장신구 강화 주문서",img:CONSUMABLES[2].img,sub:"일반~레어 장신구 강화",price:500,type:"normal"},{nm:"고급 장신구 강화 주문서",img:CONSUMABLES[2].img,sub:"에픽~신화 장신구 강화",price:2000,type:"adv"}],
  };

  const buy=(item)=>{
    const q=qty[item.type]; const total=item.price*q;
    if(gold<total){alert("골드가 부족합니다!");return;}
    onSpendGold(total); onAddScrolls(item.type,q); alert(`${item.nm} ${q}개 구매 완료!`);
  };
  ...
  return(
    <div style={{background:"#050608",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      ...
      <div style={{position:"relative", height:"90vh"}}>
        ...
        {screen==="inv"  &&<InvScreen charId={charId||"CHR_001"} charName={charName||"모험가"} gold={gold} onAddGold={addGold} scrolls={scrolls} dailyLeft={dailyLeft} onChest={()=>setShowChest(true)} onShop={()=>setShowShop(true)} onRank={()=>setShowRank(true)} onEnhance={i=>setEnhItem(i)} inventory={inventory} equipped={equipped} onEquip={equipItem} onSellItem={sellItem}/>}
